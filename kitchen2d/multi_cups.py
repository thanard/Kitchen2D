...
class MultiCups(object):
    def __init__(self):
        ...
        self.x_range = np.array(
            [[0., -10., 1., np.pi/2, 4., 4., 3., 3., 3., 3.], 
            [1., 10., 10., np.pi, 5., 5., 5., 5., 5., 5.]])
    ...
    def check_legal(self, x, n_timesteps):
        grasp_ratio, rel_x, rel_y, dangle, cw1, ch1, cw2, ch2, cw3, ch3 = x
        # print(x)
        import ipdb
        ipdb.set_trace()
        ...
        cup3 = ks.make_cup(kitchen, (15,0), 0, cw3, ch3, 0.5, user_data='cup2')
        ...
        self.cup3 = cup3
        return True
    def sampled_x(self, n, n_timesteps):
        i = 0
        while i < n:
            x = np.random.uniform(self.x_range[0], self.x_range[1])
            legal = self.check_legal(x, n_timesteps)
            if legal:
                i += 1
                yield x
    ...
    def setup(self, x, n_timesteps, image_name=None):
        '''
...
if __name__ == '__main__':
    ...
    N = 10
    n_timesteps = 50
    samples = func.sampled_x(N, n_timesteps)
    x = list(samples)
    for xx in x:
        start = time.time()
        print(func.setup(xx, n_timesteps))
